...
public class PinLogic : DefaultPinLogic<Marker, GoogleMap>
{
    ...
    protected override void OnUpdatePosition(Pin outerItem, Marker nativeItem)
    {
        ...
        {
            var oldPosition = nativeItem.Position;
            var newPosition = outerItem.Position.ToLatLng();
            ...
            // Animate marker translation
            var valueAnimator = ValueAnimator.OfFloat(0, 1);
            var startLat = oldPosition.Latitude;
            var startLng = oldPosition.Longitude;
            var endLat = newPosition.Latitude;
            var endLng = newPosition.Longitude;

            valueAnimator.SetDuration(outerItem.TranslateAnimation > 0 ? (long)outerItem.TranslateAnimation : 2500); // ms
            valueAnimator.Update += (s, e) =>
            {
                var v = (float)e.Animation.AnimatedValue;
                var lat = startLat + (endLat - startLat) * v;
                var lng = startLng + (endLng - startLng) * v;
                nativeItem.Position = new LatLng(lat, lng);
            };
            valueAnimator.AnimationEnd += (s, e) =>
            {
                nativeItem.Position = newPosition;
            };
            valueAnimator.Start();
        }
    }
    ...
    {
        if (outerItem == null || nativeItem == null)
            return;

        float startRotation = nativeItem.Rotation;
        float endRotation = outerItem.Rotation;
        float duration = outerItem.RotateAnimation > 0 ? outerItem.RotateAnimation * 1000f : 2500f; // ms

        // If the rotation hasn't changed, do nothing
        if (Math.Abs(startRotation - endRotation) < 0.01f)
        {
            nativeItem.Rotation = endRotation;
            return;
        }

        var valueAnimator = ValueAnimator.OfFloat(startRotation, endRotation);
        valueAnimator.SetDuration((long)duration);
        valueAnimator.Update += (s, e) =>
        {
            var v = (float)e.Animation.AnimatedValue;
            nativeItem.Rotation = v;
        };
        valueAnimator.AnimationEnd += (s, e) =>
        {
            nativeItem.Rotation = endRotation;
